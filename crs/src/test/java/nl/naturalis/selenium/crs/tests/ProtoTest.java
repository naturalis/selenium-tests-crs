...
public class ProtoTest extends AbstractTest {
	...
	//
	// String lastItem =
	// addMenu.getMenuItems().get(addMenu.getMenuItems().size() - 1);
	...
	// assertThat(driver.getTitle().trim(),
	// containsString(detailBeschrijvingenPage.getPageTitle()));
	// }
	//
	// @Test(priority = 6, dependsOnMethods = { "detailPageOpen" })
	// public void openFormSelect() {
	// List<WebElement> availableOptions =
	// detailBeschrijvingenPage.clickFormulierenSelect();
	...
	//
	// /**
	// * 2.1.1 Kies via "Kies een alternatief formulier" het te testen
	// formulier.
	// * (Als het te testen formulier default al geopend is, kies dan een
	// andere.)
	// * Laadt deze?
	// */
	...
	// Assert.assertEquals(detailBeschrijvingenPage.findSelectedFormulier(),
	// formListLabels.get(0), "Fout in 2.1.1");
	// }
	//
	//
	//
	...
	// * Verschijnt er een waarschuwingsteken met 'invalid value' bij het
	...
	// Assert.assertEquals(ProtoTest.detailBeschrijvingenPage.getWarningStandardStorageUnitErrorIcon(),
	// "Invalid value", "Fout in 2.1.15.01");
	//
	...
	// Assert.assertEquals(ProtoTest.detailBeschrijvingenPage.getWarningTemporaryStorageUnitErrorIcon(),
	// "Invalid value", "Fout in 2.1.15.02");
	// }
	//
	// /**
	// * 2.1.16
	...
	// Assert.assertEquals(ProtoTest.detailBeschrijvingenPage.numberOfSuggestsStandardStorageUnit("BE"),
	// 10, "Fout in 2.1.16");
	// ProtoTest.detailBeschrijvingenPage.deleteStandardStorageUnit();
	// }
	//
	...
	// Assert.assertEquals(ProtoTest.detailBeschrijvingenPage.getAttributeStandardStorageLocation("background-color"),
	// "rgba(128, 128, 128, 1)", "Fout in 2.1.17.01");
	// // 2. kan niet worden ingevoerd?
	// Assert.assertFalse(ProtoTest.detailBeschrijvingenPage.isStandardStorageLocationEnabled(),
	// "Fout in 2.1.17.02");
	...
	// Assert.assertEquals(ProtoTest.detailBeschrijvingenPage.getAttributeTemporaryStorageLocation("background-color"),
	// "rgba(128, 128, 128, 1)", "Fout in 2.1.17.03");
	// // 4. kan niet worden ingevoerd?
	// Assert.assertFalse(ProtoTest.detailBeschrijvingenPage.isTemporaryStorageLocationEnabled(),
	// "Fout in 2.1.17.04");
	// }
	//
	// /**
	...
	// Assert.assertEquals(ProtoTest.detailBeschrijvingenPage.getWarningStandardStorageLocationErrorIcon(),
	...
	// Assert.assertEquals(ProtoTest.detailBeschrijvingenPage.getWarningTemporaryStorageLocationErrorIcon(),
	...
	// * Verschijnt de autosuggest ?
	// */
	// @Test(priority = 38, dependsOnMethods = {
	// "checkIllegalValuesStorageLocations" })
	// public void checkAutosuggestStorageLocations() {
	// Assert.assertEquals(ProtoTest.detailBeschrijvingenPage.numberOfSuggestsStandardStorageLocation("DW"),
	// 10, "Fout in 2.1.19.01");
	// Assert.assertEquals(ProtoTest.detailBeschrijvingenPage.numberOfSuggestsTemporaryStorageLocation("DW"),
	// 10, "Fout in 2.1.19.01");
	//
	// ProtoTest.detailBeschrijvingenPage.deleteStandardStorageLocation();
	// ProtoTest.detailBeschrijvingenPage.deleteTemporaryStorageLocation();
	...
	// */
	// @Test(priority = 39, dependsOnMethods = {
	// "checkAutosuggestStorageLocations" })
	...
	// Assert.assertEquals(popupStorageLocations.getTitle(), "Selecteer", "Fout
	// in 2.1.20.01");
	//
	// // 2. Check whether there are storage locations
	// List<WebElement> listStorageLocations =
	// popupStorageLocations.getStorageLocations();
	// int i = 0;
	// for (WebElement element : listStorageLocations) {
	// // System.out.println(element.getText());
	// Assert.assertEquals( (element.getText().length() > 0) , true, "Fout in
	// 2.1.20.02");
	// i++;
	// }
	...
	// "checkSelectButtonStorageLocations" })
	...
	// Assert.assertEquals(popupStorageLocations.getCurrentPage(), "2", "Fout in
	// 2.1.21.02");
	// Assert.assertTrue(popupStorageLocations.searchBoxAvailable(), "Fout in
	// 2.1.21.03");
	...
	// * 2.1.22
	// *
	// * Klik in de pop-up van storage location door de boom heen en selecteer
	// een storage
	...
	// Assert.assertTrue(popupStorageLocations.selectStorageLocation(), "Fout in
	// 2.1.22");
	// }
	//
	// /**
	...
	// * 1. Is de geselecteerde storage location nu ingevoerd in het gekozen
	// veld
	// * (Standard of Temporary) storage location?
	// *
	// */
	//
	// @Test(priority = 42, dependsOnMethods = {
	// "checkSelectingStorageLocations" })
	...
	// Assert.assertEquals(ProtoTest.detailBeschrijvingenPage.numberOfWindows(),
	// 1, "Fout in 2.1.23.01");
	//
	...
	// String stringStandardStorageLocation = (String) js.executeScript("return
	// document.querySelectorAll('[name=\"CURRENTSTORAGELOCATION\"]')[0].value;");
	// Assert.assertTrue(stringStandardStorageLocation.substring( 0, 2
	// ).equals("DW"), "Fout in 2.1.23.02");
	// }
	//
	// /**
	// * 2.1.24
	...
	// Assert.assertEquals(ProtoTest.detailBeschrijvingenPage.getAttributeStandardStorageUnit("background-color"),
	// "rgba(128, 128, 128, 1)", "Fout in 2.1.24.01");
	// // 2. kan niet worden ingevoerd?
	// Assert.assertFalse(ProtoTest.detailBeschrijvingenPage.isStandardStorageUnitEnabled(),
	// "Fout in 2.1.24.02");
	//
	// //Of bij het invoeren van Temporary storage location
	...
	// ProtoTest.detailBeschrijvingenPage.setTemporaryStorageLocation("TAB");
	// // 3. kleurt dan Temporary storage unit grijs en
	// Assert.assertEquals(ProtoTest.detailBeschrijvingenPage.getAttributeTemporaryStorageUnit("background-color"),
	// "rgba(128, 128, 128, 1)", "Fout in 2.1.24.01");
	// // 4. kan niet worden ingevoerd?
	// Assert.assertFalse(ProtoTest.detailBeschrijvingenPage.isTemporaryStorageUnitEnabled(),
	// "Fout in 2.1.24.02");
	// }
	//
	// /**
	...
	// * sla het record op.
	// * 1. Lukt dit?
	// * 2. Noteer in het veld invoerwaarde de tijd tussen het op save klikken
	// en het verdwijnen van het saving blok
